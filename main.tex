...
\subsection*{3.1. Lemma 3.2 (precise version and proof sketch)}
...
\]
with $|b^{(m)}_\ell|\ll\tau_k(\ell)$ and a fixed smooth weight $\psi_m(\ell)=\psi(m\ell/N)$. Split characters into non-pretentious and exceptional by the pretentious distance $\mathbb D(1,\chi;L)$.

\emph{Non-pretentious part.} By the smooth Halász theorem with divisor weights (Granville–Soundararajan; uniform in smooth weights), for any $C\ge 1$,
\[
\sum_{\ell\asymp L} b^{(m)}_\ell\,\lambda(\ell)\,\chi(\ell)\ \ll_{k}\ L(\log L)^{-C}
\quad\big(\chi\notin\mathcal E(L;C)\big).
\]
Summing trivially over $\chi$ gives $\ll Q^2 L^2(\log N)^{-2C}$.

\emph{Exceptional part.} Let $\mathcal E_{\le Q}(L;C)$ be the union of exceptional characters up to modulus $Q$. A log-free zero-density estimate in pretentious form (e.g. Montgomery–Vaughan/Gallagher style) implies that, for any $C_1$, there exists $C_2(C_1)$ such that
\[
\#\mathcal E_{\le Q}(L;C_1)\ \ll\ Q\,(\log (QL))^{-C_2}
\qquad\text{uniformly for } Q\le L^{1/2}(\log L)^{-100}.
\]
For such $\chi$, a divisor-weighted partial summation bound yields
\[
\Big|\sum_{\ell\asymp L} b^{(m)}_\ell\,\lambda(\ell)\,\chi(\ell)\Big|\ \ll_{k}\ L(\log N)^{O(1)},
\]
whence a total contribution $\ll Q L^2 (\log N)^{-C_2+O(1)}$.

\emph{Reinsert $m$.} Multiply by $\sum_{m\sim M}|\alpha_m\lambda(m)|^2\ll M(\log N)^{O(1)}$ and use $ML=N$. Choosing $C$ then $C_2$ large in terms of $A,k,\eta$ gives
\[
\sum_{q\le Q}\sum_{\chi}|S(\chi)|^2\ \ll\ \frac{NQ}{(\log N)^A}.
\]
Summing the $O((\log N)^{O(1)})$ dyadic partitions completes the proof. The coprimality gate and restriction to primitive characters only improve or cost at most $(\log N)^{O(1)}$, absorbed in $A$.

\emph{Range condition.} The sole restriction on $Q$ is $Q\le L^{1/2}(\log L)^{-100}$ to invoke the log-free density count; with $L\ge N^{\eta}$ this is ensured by $Q\le N^{1/2}(\log N)^{-B}$ for large $B=B(A,k,\eta)$.

\emph{Remarks.} All implied constants are uniform in the fixed smooth profile $\psi$ (through finitely many derivative bounds), the divisor-weight parameter $k$, and the Type I/II ranges determined by $\eta$.
...
\paragraph{Proof sketch.}
Open the square and Poisson-sum (or apply the Kuznetsov/Deshouillers–Iwaniec bilinear form) in the $m$-variable at modulus $C$; the smoothness of $W_{q,r}$ gives negligible tails. The diagonal $\Delta_1=\Delta_2$ contributes $\ll X(P+C)$ by trivial bounds for $\sum_{m\asymp X}|W|^2$ and the size of the window in $\Delta$. For the off-diagonal, use orthogonality modulo $C$ to detect $m$ and invoke Weil’s bound $|S(u,v;C)|\le (u,v,C)^{1/2}\,\tau(C)\,C^{1/2}$, together with dyadic decomposition in $m$ and Cauchy–Schwarz in the Kloosterman variables. The derivative bounds in $W_{q,r}$ control integration by parts, yielding an extra $C^{-1}$ when $c$ is far from the stationary range; the $c\equiv 0\pmod q$ constraint is absorbed by writing $C=qr$ and summing over $r\asymp X^{1/2}/Q^2$. Collecting contributions yields the stated $(P+C)\,C^{1+2\varepsilon}\,X^{1+2\varepsilon}$ bound.

\emph{Uniformity.} The implicit constants depend only on finitely many derivatives of the fixed profile underlying $W_{q,r}$ and on $\varepsilon$; they are uniform in $q,r$ in the range specified above.
