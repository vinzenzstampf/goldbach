...
\paragraph{Step 4: Group by short prime shift and apply the $\Delta$-second moment.}
Let $\Delta=p_1-p_2$; define $\nu(\Delta)=\#\{(p_1,p_2)\in\mathcal P^2: p_1-p_2=\Delta,\ p_1\ne p_2\}$.
Then $\sum_{|\Delta|\le P}\nu(\Delta)\asymp |\mathcal P|^2$ and $\nu(\Delta)\le|\mathcal P|$.
Absorbing the kernel into a smooth weight $W_{q,r}(m,\Delta)$
with the derivative bounds of Lemma~\ref{lem:delta-second-moment}, we bound
\[
	\mathrm{OD}\ \ll\ \sum_{q\in\mathcal Q}\ \sum_{r\asymp R}\frac{1}{qr}
	\sum_{\Delta\ne 0}\nu(\Delta)\,
	\bigg|\sum_{m\asymp X} S(m,m+\Delta;qr)\,W_{q,r}(m,\Delta)\bigg|.
\]
...
\]
Since $qr\asymp C=X^{1/2}/Q$, we have $(P+qr)^{1/2}\asymp (P+X^{1/2}/Q)^{1/2}$ and
$\sum_{r\asymp R} r^{-1/2+\varepsilon}\asymp R^{1/2+\varepsilon}$.
Using $q^{-1/2}R^{1/2}\asymp Q^{-1}$,
\[
	\sum_{r}\cdots \ \ll_\varepsilon\ |\mathcal P|\,Q^{1+\varepsilon}\,(P+X^{1/2}/Q)^{1/2}.
\]
Summing over $q\in\mathcal Q$ (there are $\asymp Q$ such moduli) gives
\begin{equation}\label{eq:ODprebound}
	\mathrm{OD}\ \ll_\varepsilon\ |\mathcal P|\,Q^{2+\varepsilon}\,(P+X^{1/2}/Q)^{1/2}.
\end{equation}

\paragraph{Step 5: Divide out the amplifier and optimize exponents.}
Insert \eqref{eq:ODprebound} into \eqref{eq:amp-dom}:
\[
	\sum_{q,\chi,f}|S_{q,\chi,f}|^2
	\ \le\ \frac{1}{|\mathcal P|^2}\,\mathrm{OD}
	\ \ll_\varepsilon\ \frac{Q^{2+\varepsilon}}{|\mathcal P|}\,(P+X^{1/2}/Q)^{1/2}.
\]
Recall $|\mathcal P|\asymp \frac{P}{\log P}=X^{\vartheta+o(1)}$ and $P=X^\vartheta$.
We consider two regimes.

\medskip
\noindent\emph{Case A: $X^{1/2}/Q \le P$ (i.e. $X^{1/2-\vartheta}\le Q$).}
Then $(P+X^{1/2}/Q)^{1/2}\asymp P^{1/2}=X^{\vartheta/2}$ and
\[
	\sum_{q,\chi,f}|S_{q,\chi,f}|^2
	\ \ll_\varepsilon\ Q^{2+\varepsilon}\,X^{-\vartheta/2+o(1)}.
\]
Since $Q\le X^{1/2-\kappa}$, we get
\[
	Q^{2}\le X^{1-2\kappa},\qquad
	\sum_{q,\chi,f}|S_{q,\chi,f}|^2
	\ \ll_\varepsilon\ X^{1-2\kappa-\vartheta/2+\varepsilon}.
\]

\medskip
\noindent\emph{Case B: $X^{1/2}/Q \ge P$ (i.e. $X^{1/2-\vartheta}\ge Q$).}
Then $(P+X^{1/2}/Q)^{1/2}\asymp (X^{1/2}/Q)^{1/2}=X^{1/4}Q^{-1/2}$ and
\[
	\sum_{q,\chi,f}|S_{q,\chi,f}|^2
	\ \ll_\varepsilon\ Q^{2+\varepsilon-1/2}\,X^{1/4}\,X^{-\vartheta+o(1)}
	\ =\ Q^{3/2+\varepsilon}\,X^{1/4-\vartheta+o(1)}.
\]
Using $Q\le X^{1/2-\kappa}$,
\[
	Q^{3/2}\le X^{3/4-\tfrac{3}{2}\kappa},\qquad
	\sum_{q,\chi,f}|S_{q,\chi,f}|^2
	\ \ll_\varepsilon\ X^{(3/4-\tfrac{3}{2}\kappa)+(1/4-\vartheta)+\varepsilon}
	\ =\ X^{1-\vartheta-\tfrac{3}{2}\kappa+\varepsilon}.
\]

\paragraph{Step 6: Unify and express as $(Q^2+X)^{1-\delta}$.}
Since $Q^2\le X$, we have $Q^2+X\asymp X$, so it suffices to record an exponent saving
$X^{1-\delta}$ with
\[
	\delta\ \le\ \min\Big\{2\kappa+\frac{\vartheta}{2},\ \vartheta+\frac{3}{2}\kappa\Big\}.
\]
To ensure robustness across dyadic leaks and neglected edge terms, and to absorb any
$X^\varepsilon$ losses, we may take
\[
	\boxed{\ \ \delta\ :=\ \frac{1}{1000}\,\min\Big\{\kappa,\ \tfrac12-3\vartheta\Big\}\ }.
\]
Indeed, $\vartheta<\tfrac16-\kappa$ implies $\tfrac12-3\vartheta>3\kappa$, hence
$\min\{2\kappa+\tfrac{\vartheta}{2},\ \vartheta+\tfrac{3}{2}\kappa\}\ \ge\ c\,\min\{\kappa,\tfrac12-3\vartheta\}$
for an absolute $c>0$; shrinking by $10^3$ covers uniformity, oldforms/Eisenstein splitting,
...
\[
	...
	\ \asymp\ (Q^2+X)^{1-\delta}\,X^{\varepsilon},
\]
which is \eqref{eq:TypeIIIgoal}. Inserting this into \eqref{eq:amp-dom} yields the asserted
off-diagonal bound of Lemma~S2.4:
\[
	\mathrm{OD}\ \ll_\varepsilon\ (Q^2+X)^{1-\delta}\,|\mathcal P|^{\,2-\delta}\,X^{\varepsilon},
\]
since multiplying back by $|\mathcal P|^2$ (undoing the amplifier division) produces the stated $|\mathcal P|^{2-\delta}$ factor.
\qed
