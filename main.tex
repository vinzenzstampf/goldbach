...
\begin{proof}[Proof of Theorem~\ref{thm:BVP2M}]
	...
	$\ll NQ(\log N)^{-A}$ for any fixed $A>0$, uniformly for $Q\le N^{1/2-\varepsilon}$; see \cite[§28]{IK}, \cite[Ch.~28]{MV}.
\end{proof}
...
\begin{lemma}[Minor-arc mean square via Gallagher-type inequality]
	...
	\]
	Let $B(\alpha)=\sum_{n\asymp N} b_n\,e(n\alpha)$ be the Major-Arc Model used in \ref{part:intro}, with coefficients $b_n$ supported on $n\asymp N$ and satisfying the divisor-type bounds and smoothness properties listed in \textup{B2/B3} % TODO fix ref
...
\begin{proof}
	...
	gives an extra $(Q/|h|)$ factor. See \cite[§3.2 and Lemma~3.2]{DeshouillersIwaniec1982},
...
\begin{proof}[Proof sketch with references]
	Insert the bilinear structure, fix $u,u'$ and consider the inner correlation in $v$ with shift $h/(u)$;
	after dyadic partitioning in $u,u'$ and $v$, one arrives at shifted bilinear sums
	\[
		\sum_{v\sim V}\beta_v\,\overline{\beta_{v+h'}}\,W(v;u,u',h'),\qquad h'=\frac{h}{(u,u')},
	\]
	with smooth weights $W$ coming from the partition. Introduce a balanced prime amplifier (killing the diagonal),
	expand, and apply Kuznetsov together with the spectral large sieve. The Kloosterman-Bessel transform places the
	modulus scale $\asymp U$ (or $UV/N$) so that for $|h|\le H$ the Bessel argument is at most $O(1)$ after choosing
	the amplifier length at the standard square-root scale; one then obtains a power saving $N^{-\eta}$ in the
	$u$/$v$-aspect on average over $|h|\le H$. This is standard since \cite{DeshouillersIwaniec1982}; see also
	\cite[Thm.~16.2, Thm.~16.3]{IK}, \cite{DukeFriedlanderIwaniec1993, DukeFriedlanderIwaniec1994},
	\cite{FouvryIwaniec1997}. Your Appendix amplifier lemmas provide exactly the diagonal removal and weight smoothing
	needed; the $\tau^{O(1)}$-bounds absorb into $(\log N)^B$.
\end{proof}
...
\begin{proof}[Sketch]
	By Cauchy-Schwarz and \eqref{eq:KQ-tail} with $A=3$,
	\[
		\sum_{|h|>H} |K_Q(h)C(h)|\ \ll\ \Big(\sum_{|h|>H} |K_Q(h)|\Big)^{1/2}\Big(\sum_{|h|>H} |K_Q(h)|\,|C(h)|^2\Big)^{1/2}.
	\]
	For Type~I coefficients (one variable $\ll N^{1/2-\delta}$), expand $C(h)$ and bound the $|C(h)|^2$-sum by the
	Montgomery large sieve / BDH variance bound in progressions (see \cite[Ch.~7]{MV}, \cite[§28]{IK}), together with your
	pretentious split removing the exceptional characters uniformly up to $Q\le N^{1/2-\varepsilon}$. The resulting mean
	square is $\ll N^{1+o(1)}$ per $h$, and the kernel tail gives a total $\ll NQ(\log N)^{A}$.
\end{proof}
