...
Applying Kuznetsov (Lemma~\ref{lem:kuznetsov-uniform}) with test $h_Q(t)=h(t/Q)$ and using Lemma~\ref{lem:kernel-localization},
...
\]
where $c=qr$, $r\asymp R:=X^{1/2}/Q^{2}$ due to Lemma~\ref{lem:kuznetsov-uniform},
...
\subsection{Kuznetsov at level $q$ with level-uniform kernel bounds}
...
\subsection{Hecke $p\mid n$ tails are negligible}\label{sec:hecke-tails}
