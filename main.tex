...
\begin{lemma}[Uniform kernel localization and derivatives]\label{lem:kernel-localization-full}
...
\begin{lemma}[Δ-second moment for Kloosterman-weighted sums]\label{lem:delta-second-moment-full}
...
\begin{lemma}[Kuznetsov on $\Gamma_0(q)$, exact normalization]\label{lem:kuz}
	...
	\]
	where $\mathcal H(h)$ is the diagonal transform and $\mathcal W_h$ are the Bessel kernels described in Lemma~\ref{lem:kernel-localization-full}. All implied constants are absolute and the identity holds uniformly in $q$.
\end{lemma}
...
\begin{proof}[Proof using Lemma~\ref{lem:S2.4-full}]
