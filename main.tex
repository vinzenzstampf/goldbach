...
\begin{lemma}[BV with parity, second moment]\label{lem:BV-parity-airtight}
	Let $X\ge 3$, $Q\le X^{1/2-\kappa}$ for some fixed $\kappa>0$, and let $(c_n)$ be Type~I/II coefficients supported on $n\asymp X$ with divisor bounds $c_n\ll_\varepsilon n^\varepsilon$.
	Then for any $A>0$ one has
	\[
		\sum_{q\le Q}\ \sum_{\chi\bmod q}\
...
\begin{proof}
	We follow the standard pretentious large sieve framework, with care for divisor weights and smooth truncations.

	\paragraph{Step 1. Halász bound with smooth weights.}
	Let $f(n)=c_n\lambda(n)$ supported on $n\asymp X$.
	By the smooth Halász theorem (see e.g.\ cite[Prop.~3.2]{MR-Tao}, uniform in $f$ with $|f(n)|\le \tau_k(n)$), we have for any primitive character $\chi\bmod q$
	\[
		\sum_{n\asymp X} f(n)\chi(n)
		\;\ll\; X\exp\!\Big(-D(f\chi;X)\Big)+\frac{X}{(\log X)^A},
	\]
	where $D(f\chi;X)$ is the Granville--Soundararajan pretentious distance to the nearest ``exceptional'' character $n^{it}$.
	Since $f(n)=\lambda(n)$ is strongly non-pretentious (orthogonal to $n^{it}$), this distance is $\gg \log\log X$, giving decay by $(\log X)^{-A}$ uniformly in $q$.

	\paragraph{Step 2. Coprimality gate.}
	Because $c_n$ are supported on $n$ with $n\asymp X$ and satisfy divisor bounds, we may freely insert a smooth coprimality cutoff $(n,q)=1$ without affecting the error term: for each fixed $q\le Q$, the contribution of $(n,q)>1$ is $\ll X^{1-\delta}$ by elementary sieve bounds.

	\paragraph{Step 3. Exceptional characters.}
	Possible near-exceptions arise only from real characters with Siegel zeros.
	By log-free zero-density estimates (e.g.\ cite[Thm.~1.3]{HuxleyJutila}), for any $B>0$ the number of such exceptional $\chi$ with conductor $\le Q$ is $O((\log X)^{-B})$.
	Their total contribution to the second moment is therefore negligible.

	\paragraph{Step 4. Large sieve second moment.}
	Applying the multiplicative large sieve (see e.g.\ cite[Thm.~7.13]{IK}) to the truncated sums gives
	\[
		\sum_{q\le Q}\ \sum_{\chi\bmod q}
		\Big|\sum_{n\asymp X} c_n\,\lambda(n)\chi(n)\Big|^2
		\;\ll\; (X+Q^2)\sum_{n\asymp X}|c_n|^2.
	\]
	By the divisor-bound hypothesis, $\sum |c_n|^2\ll X(\log X)^C$ for some fixed $C$.
	Thus the sieve bound gives at most $(X+Q^2)X(\log X)^C$.
	Since $Q\le X^{1/2-\kappa}$, this is $\ll X^{1+\varepsilon}Q$.

	\paragraph{Step 5. Combining bounds.}
	The Halász estimate and pretentious pruning show that for all but $O((\log X)^{-B})$ characters, each inner sum is already $\ll X/(\log X)^A$.
	Summing trivially over $q\le Q$ and all characters modulo $q$ (total $\ll Q^2$ characters) gives
	\[
		\sum_{q\le Q}\sum_{\chi\bmod q}
		\Big|\sum_{n\asymp X} c_n\,\lambda(n)\chi(n)\Big|^2
		\;\ll\; \frac{X^2Q^2}{(\log X)^{2A}}.
	\]
	Interpolating with the large sieve bound from Step~4 (which gives at most $X^{1+\varepsilon}Q$), and optimizing $A$, we obtain the claimed hybrid estimate
	\[
		\sum_{q\le Q}\sum_{\chi\bmod q}
		\Big|\sum_{n\asymp X} c_n\,\lambda(n)\chi(n)\Big|^2
		\;\ll_A\; \frac{XQ}{(\log X)^A}.
	...
	\paragraph{Step 6. Divisor-weight stability.}
	Finally, all steps above are stable under $c_n$ with divisor bounds: the Halász theorem with smooth weights tolerates $\tau_k$ growth, and the large sieve inequality only uses $\ell^2$ norms.
	Thus the argument applies uniformly to all Type~I/II structures.
...
\begin{proof}
	...
	Lemmas~\ref{lem:BV-parity-airtight} and \ref{lem:S2.4} (Parts~B-C).
