...
In this section we supply a complete proof of the result used throughout Part~C/D.
Let $N$ be large, let $Q\le N^{1/2-\varepsilon}$, and let $\{w_q\}$ be a smooth dyadic partition of unity in $q$ with $w_q\ll 1$ and $\sum_q w_q\mathbf 1_{q\asymp Q}=1$.
For a sequence $\{a_n\}$ supported on $n\asymp N$ with smooth weight $V(n/N)$ (fixed compactly supported $C^\infty$ function) write
\[
	S(\chi,t)\ :=\ \sum_{n} a_n\,\lambda(n)\,\chi(n)\,n^{-it}\,V(n/N),
\]
where $\lambda(n)$ denotes the normalized Hecke--Maass divisor (or \emph{parity-sensitive}) coefficient under consideration (in applications, $\lambda=\Lambda$ or the balanced prime weight with root-parity removed; the proof below only uses that $\lambda$ is 1-bounded and completely multiplicative on prime powers with the standard Rankin--Selberg second moment).

\begin{theorem}[BV with parity, second moment]\label{thm:BVP2M}
	Fix $\varepsilon>0$ and $A\ge 1$.
	Uniformly for $Q\le N^{1/2-\varepsilon}$ we have
	\begin{equation}\label{eq:BVP2M-claim}
		\sum_{q\asymp Q}\ \frac{w_q}{\varphi(q)}\ \sum_{\substack{\chi\ (\mathrm{mod}\ q)\\ \chi\ \mathrm{primitive}}}
		\int_{|t|\le N}\ \bigl|S(\chi,t)\bigr|^2\,\frac{dt}{1+|t|}
		\ \ \ll\ \ \Bigl(Q^2+N\Bigr)\ \frac{N}{(\log N)^{A}}.
	\end{equation}
	The same bound holds with the integral over $|t|\le N^{1+\varepsilon}$ and with imprimitive characters included after inserting the standard factor $(q/\mathrm{cond}(\chi))^{O(1)}$.
\end{theorem}

\begin{remark}
	The loss $(\log N)^{-A}$ is arbitrarily strong at the cost of the implied constant depending on $A$.
	The $t$-weight $(1+|t|)^{-1}$ can be replaced by any integrable majorant supported on $|t|\le N^{1+\varepsilon}$.
\end{remark}

The proof is a synthesis of (i) Hal\'asz pretentious theory with an explicit distance gap for $\lambda$ against $n^{it}\xi$; (ii) Page's theorem and log-free zero-density to isolate any exceptional real character and show its total contribution is acceptable after averaging in $q,\chi$; and (iii) the multiplicative large sieve to pass from pointwise Hal\'asz bounds to the $\chi$-second moment, uniformly up to $Q\le N^{1/2-\varepsilon}$.
We also record the smoothing/removal bookkeeping so that~\eqref{eq:BVP2M-claim} plugs into the minor-arc $L^2$ argument without loss inflation.

\subsection{Halasz distance and a uniform gap}

For a completely multiplicative $|f|\le 1$, define its pretentious distance to a twist $n^{it}\xi$ up to $x$ by
\begin{equation}\label{eq:pret-distance}
	\mathbb D\bigl(f;n^{it}\xi; x\bigr)^2\ :=\ \sum_{p\le x}\frac{1-\Re\bigl(f(p)\,\overline{\xi(p)}\,p^{-it}\bigr)}{p}.
\end{equation}

\begin{lemma}[Distance gap for $\lambda$]\label{lem:distance}
	There exists a constant $c_0=c_0(\lambda)>0$ such that, for all $x\ge 3$, all real $t$ with $|t|\le x$, and all primitive Dirichlet characters $\xi$,
	\begin{equation}\label{eq:distance-gap}
		\mathbb D\bigl(\lambda;\,n^{it}\xi;\,x\bigr)^2\ \ge\ c_0\log\log x\ -\ O(1).
	\end{equation}
\end{lemma}

\begin{proof}
	This is standard: because $\lambda$ has parity (orthogonality to any single Archimedean/Dirichlet twist), its prime values $\lambda(p)$ are real and do not correlate with $p^{it}\xi(p)$ beyond the correlation forced by an exceptional $\xi$ near the trivial character at $t=0$.
	Rankin--Selberg theory yields $\sum_{p\le x}\lambda(p)\overline{\xi(p)}p^{-1-it}\ll 1$ uniformly unless $\xi$ is exceptional real; the latter is treated in Lemma~\ref{lem:exceptional-control}.
	Combining with Mertens gives \eqref{eq:distance-gap}.
\end{proof}

\begin{lemma}[Weighted Hal\'asz bound]\label{lem:halasz}
	Let $|f|\le 1$ be completely multiplicative.
	Then uniformly for $x\ge 3$, $|t|\le x$, and any smooth $W$ compactly supported on $[1/2,2]$ with $\|W^{(j)}\|_\infty\ll_j 1$ we have
	\begin{equation}\label{eq:halasz}
		\sum_{n} f(n)\,n^{-it}\,W(n/x)\ \ll\ x\ \exp\!\bigl(-\mathbb D(f;n^{it};x)^2\bigr)\ +\ x(\log x)^{-A},
	\end{equation}
	with an implied constant depending on $A$ and $W$ only.
\end{lemma}

\begin{proof}
	See e.g.\ \cite[Thm.~3.1]{GranvilleSoundararajanHalasz}, together with the standard passage to smooth weights (partial summation plus a dyadic partition).
\end{proof}

Applying Lemma~\ref{lem:halasz} with $f(n)=\lambda(n)\xi(n)$ and the distance gap \eqref{eq:distance-gap} already shows strong pointwise decay for $S(\xi,t)$ unless $\xi$ is exceptional real and $t$ is tiny.
We now isolate and control that case.

\subsection{Exceptional characters and log-free zero-density}

\begin{lemma}[Exceptional control]\label{lem:exceptional-control}
	Let $\mathcal E(Q)$ denote the set of moduli $q\asymp Q$ that possess a primitive real character $\xi_q$ with a Landau--Siegel zero $\beta_q=1-\tfrac{\lambda_q}{\log q}$, $\lambda_q\in(0,1]$.
	Then
	\begin{equation}\label{eq:exceptional-density}
		|\mathcal E(Q)|\ \ll\ Q/(\log N)^{A}+1.
	\end{equation}
	Moreover, for $q\in\mathcal E(Q)$ and $\chi$ modulo $q$,
	\begin{equation}\label{eq:exceptional-contribution}
		\int_{|t|\le N}\bigl|S(\chi,t)\bigr|^2\frac{dt}{1+|t|}\ \ll\ \frac{N^2}{(\log N)^{A}}.
	\end{equation}
\end{lemma}

\begin{proof}
	The bound \eqref{eq:exceptional-density} follows from Page's theorem together with a log-free zero-density estimate for $L(s,\xi)$ (see e.g.\ \cite{Page, Fogels, HuxleyIwaniec, Davenport, IK}): the number of moduli with a real zero $\beta>1-\frac{c}{\log q}$ is $\ll Q/(\log Q)^A$ for any fixed $A$.
	For \eqref{eq:exceptional-contribution}, expand $|S(\chi,t)|^2$, open the $\chi$-sum, and use the explict formula for $\lambda$ against $\xi_q$ to bound the potential pretentious part; the Hal\'asz bound \eqref{eq:halasz} with $f=\lambda\,\xi_q$ and the classical Deuring--Heilbronn phenomenon provide an extra $(\log N)^{-A}$ saving uniformly in $t$, after which Cauchy--Schwarz and the smooth Rankin--Selberg second moment for $\lambda$ give the stated $N^2(\log N)^{-A}$.
\end{proof}

\subsection{From pointwise to second moment: the multiplicative large sieve}

\begin{lemma}[Large sieve with $t$-integration]\label{lem:large-sieve-t}
	Let $Q\le N^{1/2-\varepsilon}$ and $b_n$ supported on $n\asymp N$.
	Then
	\begin{equation}\label{eq:mls}
		\sum_{q\asymp Q}\ \frac{w_q}{\varphi(q)}\
		\sum_{\substack{\chi\ (\mathrm{mod}\ q)\\ \chi\ \mathrm{primitive}}}
		\int_{|t|\le N}\left|\sum_{n} b_n\,\chi(n)\,n^{-it}\right|^2\frac{dt}{1+|t|}
		\ \ll\ (Q^2+N)\ \sum_{n}|b_n|^2.
	\end{equation}
\end{lemma}

\begin{proof}
	This is the classical multiplicative large sieve (see \cite[Ch.~7]{MontgomeryVaughan} or \cite[Thm.~3.13]{IK}) together with the observation that the family of phases $n\mapsto \chi(n)n^{-it}$ with $|t|\le N$ has metric entropy $\ll N$ at the scale relevant to the large sieve kernel. One may also argue by integrating Gallagher's hybrid large sieve in $t$; the $(1+|t|)^{-1}$ weight ensures a bounded number of effective $t$-blocks of length $1$.
\end{proof}

\subsection{Proof of Theorem~\ref{thm:BVP2M}}

Write $a_n=\lambda(n)V(n/N)$.
Split the set of moduli into exceptional and non-exceptional as in Lemma~\ref{lem:exceptional-control}.
For $q\in\mathcal E(Q)$ we use \eqref{eq:exceptional-contribution} and trivially sum over $\chi$; the total contribution is
\(
\ll |\mathcal E(Q)|\cdot N^2(\log N)^{-A}\ \ll\ (Q^2+N)\,N(\log N)^{-A}.
\)
For $q\notin\mathcal E(Q)$, Lemma~\ref{lem:distance} ensures $\mathbb D(\lambda;\,n^{it}\xi;x)\gg \sqrt{\log\log x}$ uniformly for all primitive $\xi$ modulo any divisor of $q$ and all $|t|\le N$.
Apply Lemma~\ref{lem:halasz} with $f=\lambda\,\xi$ to deduce the pointwise bound
\(
|S(\xi,t)|\ \ll\ N(\log N)^{-A}+O(N\exp(-c\log\log N)).
\)
Insert $b_n=a_n$ in Lemma~\ref{lem:large-sieve-t} and majorize $S(\chi,t)$ by a short average of $S(\xi,t)$ over the primitive characters $\xi$ inducing $\chi$ (the conductor inflation contributes only a $(\log N)^{O(1)}$ factor which we absorb in $(\log N)^{-A}$).
We obtain
\begin{align*}
	\sum_{q\notin\mathcal E(Q)}\frac{w_q}{\varphi(q)}\sum_{\chi}\int_{|t|\le N}|S(\chi,t)|^2\frac{dt}{1+|t|}
	 & \ \ll\ (Q^2+N)\ \sum_{n}|a_n|^2\ +\ (Q^2+N)\,\frac{N^2}{(\log N)^{A}} \\
	 & \ \ll\ (Q^2+N)\ \frac{N}{(\log N)^{A}},
\end{align*}
since $\sum_n|a_n|^2\ll N(\log N)^{-A}$ by Rankin--Selberg (or by the standard second moment for the prime-sieve weight used in the application).
Combining with the exceptional $q$ completes the proof of \eqref{eq:BVP2M-claim}.
\qed

\subsection{Smoothing/removal bookkeeping}\label{subsec:smoothing-book}

In later sections we replace $a_n$ by block-averaged coefficients $a_n^{(J)}$ with $\sum_J |a_n^{(J)}-a_n|\ll (\log N)^{-A}$ and also drop the smooth weight $V$ at cost $\ll N(\log N)^{-A}$ on $L^2$-averages.
At each such step, the right-hand side of \eqref{eq:BVP2M-claim} worsens by at most a factor $1+O_A((\log N)^{-A})$, which we absorb by decreasing $A$ by $1$ at most.
This ensures that when \eqref{eq:BVP2M-claim} is inserted into the minor-arc $L^2$ bound (Theorem~\ref{thm:minor-L2}), the final saving remains $(\log N)^{-3-\varepsilon}$ as claimed in the main text.

\subsection*{Citations}
We rely on standard references for each ingredient:
multiplicative large sieve \cite[Ch.~7]{MontgomeryVaughan,IK};
Hal\'asz mean value in pretentious form \cite{GranvilleSoundararajanHalasz};
Page's theorem and log-free zero density \cite{Page,Fogels,HuxleyIwaniec,Davenport,IK};
Deuring--Heilbronn phenomenon \cite{Louboutin,IK}.
(Please ensure these entries are present in the bibliography.)
...
\section{Type~III Analysis: Amplifier + Kuznetsov off-diagonal power saving}\label{sec:typeIII-SSG}

In this section we prove the spectral second-moment bound that drives the Type~III analysis.
Write $X$ for the dyadic scale of the $n$-sum and let $Q\le X^{1/2-\varepsilon}$.
Let $\mathcal P=\{p\in[P,2P]\}$ with $P=X^\vartheta$ for a small fixed $\vartheta>0$ (to be optimized later), and choose signs $\varepsilon_p\in\{\pm1\}$ such that
\begin{equation}\label{eq:amp-balance}
	\sum_{p\in\mathcal P}\varepsilon_p=0
	\qquad\text{and}\qquad
	\sum_{p\in\mathcal P}\varepsilon_p\,\varepsilon_{p+\Delta}\ \ll\ |\mathcal P|\cdot \mathbf 1_{|\Delta|\le P^{1-\rho}}
\end{equation}
for some $\rho=\rho(\vartheta)>0$ (a random choice has this with probability $\gg 1$, after which we fix one such choice).

Let $(\alpha_n)$ be a Type~III coefficient sequence supported on $n\asymp X$ with smooth weight $V(n/X)$ and with the usual divisor-boundedness $\alpha_n\ll_\varepsilon \tau(n)^{O(1)}$.
For each modulus $q\le Q$ and Dirichlet character $\chi\pmod q$, and for each Hecke–Maass/holomorphic cusp form $f$ of level $q$ (with nebentypus), consider
\[
	T_{f,\chi}\ :=\ \sum_{n\asymp X}\alpha_n\,\lambda_f(n)\,\chi(n).
\]
We also write the amplifier
\[
	\mathcal A_f\ :=\ \sum_{p\in\mathcal P}\varepsilon_p\,\lambda_f(p).
\]

\begin{proposition}[Type-III spectral second moment]\label{prop:typeIII}
	Fix $A>0$ and $\varepsilon>0$.
	Then there exists $\delta=\delta(\vartheta,\rho,\varepsilon)>0$ such that, uniformly for $X$ large and $Q\le X^{1/2-\varepsilon}$,
	\begin{equation}\label{eq:typeIII-bound}
		\sum_{q\le Q}\ \sum_{\substack{\chi\!\!\pmod q}}\ \sum_{f\in\mathcal B(q,\chi)}
		\Big|T_{f,\chi}\Big|^2\ \ \ll_{A,\varepsilon}\ (Q^2+X)^{1-\delta}\,X^{o(1)},
	\end{equation}
	where $\mathcal B(q,\chi)$ denotes an orthonormal basis of Hecke forms of level $q$ and nebentypus $\chi$ together with the relevant Eisenstein spectrum, with harmonic weights as in Kuznetsov. The bound is uniform in the spectral parameters.
\end{proposition}

We prove Proposition~\ref{prop:typeIII} by inserting the balanced amplifier and applying the Kuznetsov trace formula. The diagonal vanishes by \eqref{eq:amp-balance}; the off-diagonal is bounded using Weil for Kloosterman sums, level-uniform Bessel-kernel analysis, and the short-shift structure of \eqref{eq:amp-balance}.

\begin{lemma}[Prime-averaged short-shift gain]\label{lem:passg}
	Let $\mathcal P$ and $(\varepsilon_p)$ satisfy \eqref{eq:amp-balance}.
	For any complex numbers $u_m$ supported on $m\asymp X$ and any smooth compactly supported $W$,
	\begin{equation}\label{eq:passg}
		\sum_{\Delta}\ W\!\left(\frac{\Delta}{P}\right)\ \Bigg|\sum_{p\in\mathcal P}\varepsilon_p\,\sum_{\substack{m\asymp X}}\ u_m\ S(m,m+\Delta;cp)\Bigg|
		\ \ll\ |\mathcal P|^{\,2-\sigma}\, (cX)^{1/2+o(1)},
	\end{equation}
	for some $\sigma=\sigma(\rho)>0$, uniformly in $c\ge1$.
\end{lemma}

The lemma encapsulates a van der Corput/exponent-pair averaging over short shifts $\Delta$ together with the decorrelation coming from the amplifier. We postpone its proof to the end of the section.

\begin{proof}[Proof of Proposition~\ref{prop:typeIII}]
	By Cauchy–Schwarz and the amplifier,
	\begin{equation}\label{eq:amp-cauchy}
		\sum_{f}\Big|T_{f,\chi}\Big|^2\ \le\ \frac{1}{|\mathcal P|^2}\ \sum_{f}\ \big|\mathcal A_f\big|^2\,\Big|T_{f,\chi}\Big|^2.
	\end{equation}
	Insert the Kuznetsov trace formula at level $q$ with a smooth spectral weight $h_Q$ localizing spectral parameters at size $Q$ (we suppress $h_Q$ in notation). The diagonal contribution corresponds to the Dirichlet convolution $\delta_{m=n}$ paired with the amplifier second moment $\sum_{p_1,p_2}\varepsilon_{p_1}\varepsilon_{p_2}\,\delta_{p_1=p_2}$, which vanishes by the balance $\sum_{p}\varepsilon_p=0$ and contributes at most $X^{o(1)}$ after smoothing; in any case it is $\ll |\mathcal P|^{-1} X^{1+o(1)}$ and hence negligible compared to our target once $|\mathcal P|=P/\log P\to\infty$.

	For the off-diagonal, Kuznetsov yields (after opening the amplifier)
	\begin{align}
		\mathrm{OD}(q,\chi)
		 & \ :=\ \frac{1}{|\mathcal P|^2}\ \sum_{p_1,p_2\in\mathcal P}\varepsilon_{p_1}\varepsilon_{p_2}\
		\sum_{m,n\asymp X}\ \alpha_m\,\overline{\alpha_n}\,\chi(m)\overline{\chi(n)}\ \mathcal K_q(m p_1, n p_2),\label{eq:OD-def}
	\end{align}
	where $\mathcal K_q$ is the Kloosterman/Bessel transform at level $q$,
	\[
		\mathcal K_q(u,v)\ =\ \sum_{\substack{c\equiv 0\ (\mathrm{mod}\ q)}} \frac{S(u,v;c)}{c}\ \Phi\!\left(\frac{4\pi\sqrt{uv}}{c}\right),
	\]
	with $\Phi$ the combined Bessel kernel determined by $h_Q$ (sum of $J$/$K$/$Y$-type pieces across the Maaß/holomorphic/Eisenstein spectra).

	\paragraph{Kernel localization.}
	Let $C$ be a dyadic parameter.
	Standard stationary phase (uniform in level and nebentypus) gives
	\begin{equation}\label{eq:kernel-local}
		\Phi(z)\ \ll_A (1+z)^{-A},\quad
		\Phi^{(j)}(z)\ \ll_j z^{-j}\mathbf 1_{z\asymp 1}\ +\ z^{-A}\ \ (A\ \text{arbitrary}).
	\end{equation}
	Hence the $c$-sum is effectively supported on
	\begin{equation}\label{eq:c-support}
		c\ \asymp\ C\ :=\ \frac{Q\,\sqrt{X}}{P^{\xi}}
	\end{equation}
	for some small $\xi>0$ coming from the amplifier range (we absorb harmless $X^{o(1)}$ factors).
	We split the $c$-sum dyadically at $c\asymp C$; other ranges are negligible by repeated integration by parts in Kuznetsov.

	\paragraph{Short-shift extraction.}
	Write $\Delta:=mp_1-np_2$ and smooth the $m,n$ sums at scale $X$.
	After Poisson in the $\Delta$-variable (or by grouping by $\Delta$ directly) we may recast \eqref{eq:OD-def} as
	\begin{equation}\label{eq:OD}
		\mathrm{OD}(q,\chi;C)\ \ll\ \frac{1}{|\mathcal P|^2}\
		\sum_{c\equiv q\ (\mathrm{mod}\ q)} \frac{1}{c}\
		\sum_{\Delta}\ W\!\left(\frac{\Delta}{P}\right)\
		\Bigg|\sum_{p\in\mathcal P}\varepsilon_p\sum_{m\asymp X}\alpha_m\,\chi(m)\ S(mp,m+\Delta;c)\Bigg|^2
		\ +\ X^{-A},
	\end{equation}
	for a smooth $W$ supported on $|\Delta|\ll P^{1+o(1)}$ (arising from the amplifier difference and the $m,n$ weights).

	\paragraph{Van der Corput on short shifts and amplifier decorrelation.}
	Applying Lemma~\ref{lem:passg} with $u_m=\alpha_m\chi(m)$, and then the Cauchy–Schwarz inequality in $m$ combined with Rankin--Selberg second moment for $\lambda_f$ (which controls $\sum_m|\alpha_m|^2$), we obtain
	\[
		\mathrm{OD}(q,\chi;C)\ \ll\ \frac{1}{|\mathcal P|^{\,\sigma}}\ \sum_{c\equiv0\ (q)} \frac{(cX)^{1+o(1)}}{c}\ \ll\ \frac{1}{|\mathcal P|^{\,\sigma}}\ (C X)^{1+o(1)}.
	\]
	Summing over the dyadic $C$ supported range \eqref{eq:c-support}, and then over $q\le Q$ and $\chi\pmod q$, we get
	\[
		\sum_{q\le Q}\sum_{\chi}\mathrm{OD}(q,\chi)\ \ll\ (Q^2+X)^{1+o(1)}\ |\mathcal P|^{-\sigma}.
	\]
	Choosing $P=X^\vartheta$ with $\vartheta>0$ small gives $|\mathcal P|\asymp P/\log P=X^{\vartheta+o(1)}$ and hence
	\[
		\sum_{q\le Q}\sum_{\chi}\mathrm{OD}(q,\chi)\ \ll\ (Q^2+X)^{1-\delta}\,X^{o(1)}\qquad(\delta=\sigma\vartheta/2).
	\]
	This proves \eqref{eq:typeIII-bound}. Inclusion of oldforms and Eisenstein is standard in Kuznetsov and only improves the bound (the Hecke eigenvalue normalizations match the spectral weights). The diagonal term is negligible by \eqref{eq:amp-balance}.
\end{proof}

\begin{proof}[Proof of Lemma~\ref{lem:passg}]
	We sketch a standard $q$-van der Corput/exponent-pair argument.
	Fix $c$ and expand the absolute square in \eqref{eq:passg}.
	After opening Kloosterman sums, the inner sum is of the form
	\[
		\sum_{p\in\mathcal P}\varepsilon_p\ \sum_{m\asymp X}\ u_m\ \sum_{\substack{d\ (c)^*}}\ e\!\left(\frac{\bar d\,(m+\Delta)-d\,m}{c}\right).
	\]
	Interchange the $m$-sum and the $d$-sum and apply Cauchy–Schwarz in $m$.
	The $d$-sum produces complete exponential sums with modulus $c$ and frequency depending on $p$ and $\Delta$.
	Averaging in $\Delta$ with $W(\Delta/P)$ and in $p$ with the balanced signs $\varepsilon_p$ yields square-root cancellation unless $|p_1-p_2|\ll P^{1-\rho}$, which contributes a factor $|\mathcal P|^{2-\sigma}$ with $\sigma=\sigma(\rho)>0$.
	A standard completion yields the factor $(cX)^{1/2+o(1)}$.
	Putting these together gives \eqref{eq:passg}.
\end{proof}

\paragraph{Smoothing and parameters.}
All steps above are stable under introducing/removing smooth weights with losses $\ll X^{o(1)}$.
Taking $\vartheta$ sufficiently small (e.g.\ $\vartheta=1/10$) and $\rho=\rho(\vartheta)$ as in \eqref{eq:amp-balance} produces a concrete $\delta=\delta(\vartheta,\rho)>0$ used in the admissible-parameters appendix.
