...
\part{Introduction \& Framework}\label{part:intro}
...
\part{Type III Analysis}\label{part:typeIII}
...
\begin{proof}[Proof of Theorem~\ref{thm:BVP2M}]
	Start from the character–variance/dispersion identity:
	\[
		\sum_{q\le Q}\ \sum_{\chi\bmod q}\Big|\sum_{n\asymp N} c_n\,\chi(n)\Big|^2
		\ =\ \sum_{h\in\mathbb Z} K_Q(h)\,C(h),
	\]
	with $K_Q$ defined using a smooth $h_Q$ supported on $[Q/2,2Q]$ (absorbing the smoothing loss into $(\log N)^{O(1)}$).
	Split at $H=N/Q$:
	\begin{equation}\label{eq:H-choice}
		\sum_{h} K_Q(h)\,C(h)\ =\ \sum_{|h|\le H} K_Q(h)\,C(h)\ +\ \sum_{|h|>H} K_Q(h)\,C(h).
	\end{equation}
	For the short-shift range, apply Proposition~\ref{prop:kernel-short} (which uses
	Lemmas~\ref{lem:kernel-smooth} and \ref{lem:shortshift-TypeII}) to get
	\[
		\sum_{|h|\le H} K_Q(h)\,C(h)\ \ll\ Q^2\,N^{1-\eta}\,(\log N)^{B}.
	\]
	For the long-shift range, apply Lemma~\ref{lem:long-tail} (Type~I/II plus pretentious split) to obtain
	\[
		\sum_{|h|>H} K_Q(h)\,C(h)\ \ll\ NQ\,(\log N)^{A}.
	\]
	Combining the two ranges proves the claimed bound
	\[
		\sum_{q\le Q}\ \sum_{\chi\bmod q}\Big|\sum_{n\asymp N} c_n\,\chi(n)\Big|^2
		\ \ll\ (NQ\;+\;Q^2 N^{1-\eta})\,(\log N)^{O(1)}.
	\]
	Finally, in the pure Type~I regime the short-shift lemma is not needed; the pretentious analysis (your
	Lemmas~\ref{lem:halasz-smooth}, \ref{lem:logfree-density}, \ref{lem:siegel}) yields the Bombieri–Vinogradov strength
	$\ll NQ(\log N)^{-A}$ for any fixed $A>0$, uniformly for $Q\le N^{1/2-\varepsilon}$; see \cite[\S28]{IK}, \cite[Ch.~28]{MV}.
\end{proof}
...
\begin{lemma}[Minor-arc mean square via Gallagher-type inequality]
	...
	\]
	Let $B(\alpha)=\sum_{n\asymp N} b_n\,e(n\alpha)$ be the Major-Arc Model used in \ref{part:intro}, with coefficients $b_n$ supported on $n\asymp N$ and satisfying the divisor-type bounds and smoothness properties listed in Lemma~\ref{lem:beta-properties}—namely \ref{B:avg} and \ref{B:dist}
	(in particular $|b_n|\ll_\varepsilon n^\varepsilon$ and $b_n$ is a short, smooth combination of Type~I/II/III convolutions already treated in \ref{part:typeI-II}-\ref{part:typeIII})
...
\begin{lemma}[Properties of the sieve majorant]\label{lem:beta-properties}
	...
	\begin{enumerate}[leftmargin=2em]
		...
		\item \textbf{Average size: }\label{B:avg}  $\displaystyle \sum_{n}\beta(n)\,W\!\Big(\frac{n}{N}\Big)=\frac{N}{\log N}\,(1+o(1))$.
		\item \textbf{Distribution mod $q\le N^{1/2-\varepsilon}$: }\label{B:dist}  uniformly for $(a,q)=1$ and $|\beta|\le Q/(qN)$,
		      \[
...
\section{Auxiliary analytic inputs used in Part \ref{part:typeI-II}}

\begin{lemma}[Smoothed dispersion kernel]\label{lem:kernel-smooth}
	Let $h_Q:\mathbb R_{\ge 1}\to\mathbb R_{\ge 0}$ be a smooth cutoff supported on $[Q/2,2Q]$ with
	\[
		x^j h_Q^{(j)}(x)\ \ll_j\ 1\qquad (0\le j\le 4).
	\]
	Define the Ramanujan-dispersion kernel
	\[
		K_Q(h)\ :=\ \sum_{q\ge 1} h_Q(q)\,\frac{1}{\varphi(q)}\sum_{\substack{a\bmod q\\ (a,q)=1}} e\!\left(\frac{ah}{q}\right)
		\ =\ \sum_{q\ge 1} h_Q(q)\,\frac{c_q(h)}{\varphi(q)},
	\]
	where $c_q(h)$ is the Ramanujan sum. Then for any $H\ge 1$ we have, uniformly in $h\in\mathbb Z$:
	\begin{align}
		\label{eq:KQ-short}
		\max_{1\le |h|\le H}\ |K_Q(h)|
		 & \ \ll\ \frac{Q^2}{H}\ +\ Q^{1+\varepsilon},                        \\
		\label{eq:KQ-tail}
		\sum_{|h|>H}\! |K_Q(h)|
		 & \ \ll_A\ Q^2 \Big(\frac{Q}{H}\Big)^{A}\qquad\text{for any }A\ge 1.
	\end{align}
	The implied constants may depend on $\varepsilon,A$ and the derivative bounds of $h_Q$, but not on $Q,H$ or $h$.
\end{lemma}

\begin{proof}
	By orthogonality, $K_Q(h)=\sum_{q} h_Q(q)c_q(h)/\varphi(q)$. Using the Dirichlet hyperbola method on
	$\sum_{q\asymp Q} c_q(h)$ with the classical identity $c_q(h)=\sum_{d\mid (q,h)} d\,\mu(q/d)$
	and partial summation with the smooth $h_Q$, we obtain
	\[
		K_Q(h)\ \ll\ \sum_{d\mid h} d \sum_{\substack{q\asymp Q\\ d\mid q}} \frac{|\,\mu(q/d)\,|}{\varphi(q)}
		\ \ll\ \sum_{d\mid h} \frac{d}{\varphi(d)}\cdot \frac{Q}{d}\ +\ O(Q^{1+\varepsilon})
		\ \ll\ Q^{1+\varepsilon}\ +\ \sum_{d\mid h}\frac{Q}{\varphi(d)}.
	\]
	Averaging over $1\le |h|\le H$ and applying Cauchy-Schwarz to the divisor sum (together with
	$\sum_{d\le H} \varphi(d)^{-2}\ll H^{\varepsilon}$) gives the uniform bound
	$K_Q(h)\ll Q^{1+\varepsilon}+Q^2/H$, proving \eqref{eq:KQ-short}.
	For the tail \eqref{eq:KQ-tail}, apply repeated summation by parts in $q$ using the $C^4$-control on $h_Q$,
	together with the Weil bound $|c_q(h)|\le (q,h)\tau(q)$ and dyadic decomposition; every integration by parts
	gives an extra $(Q/|h|)$ factor. See \cite[\S3.2 and Lemma~3.2]{DeshouillersIwaniec1982},
	\cite[Prop.~13.2 and (16.50)]{IK} for details.
\end{proof}

\begin{lemma}[Short-shift bilinear mean saving]\label{lem:shortshift-TypeII}
	Let $N$ be large and let $c_n$ be a bilinear coefficient sequence supported on $n\asymp N$ of Type~II/III shape:
	\[
		c_n\ =\ \sum_{\substack{uv=n\\ U\le u\le 2U\\ V\le v\le 2V}} \alpha_u\,\beta_v,
		\qquad |\alpha_u|\le \tau(u)^{O(1)},\ |\beta_v|\le \tau(v)^{O(1)},\quad UV\asymp N,
	\]
	with $N^{\delta}\le U,V\le N^{1-\delta}$ for some fixed $\delta>0$.
	Define the short-shift correlations $C(h):=\sum_{n\asymp N} c_n\,\overline{c_{n+h}}$.
	Then there exist constants $\eta=\eta(\delta)>0$ and $B=B(\delta)$ such that for any $H$ with $1\le H\le N$,
	\begin{equation}\label{eq:shortshift-mean}
		\sum_{|h|\le H}\, |C(h)|\ \ll\ H\,N^{1-\eta}\,(\log N)^{B}.
	\end{equation}
\end{lemma}

\begin{proof}
	We give a parameter–explicit proof using the balanced prime amplifier (Lemma~\ref{lem:balanced-signs}) and the Kuznetsov formula with level–uniform test functions (Section~\ref{sec:kuznetsov-uniform}).

	\emph{Step 1: Dyadicising and isolating a shifted convolution.}
	Insert the bilinear structure for $c_n$ and perform a smooth dyadic partition in $u,u',v$ so that $u\sim U$, $u'\sim U$, $v\sim V$, with $UV\asymp N$ and $N^{\delta}\le U,V\le N^{1-\delta}$. Writing $n=uv$, the correlation
	\[
		C(h)\;=\;\sum_{uv\asymp N}\alpha_u\beta_v\,\overline{\alpha_{u'}\beta_{v'}}\ \mathbf 1_{u'v'=uv+h}
	\]
	reduces to sums of the shape
	\begin{equation}\label{eq:ss-shape}
		\mathcal C(h;U,V)\ :=\ \sum_{u\sim U}\sum_{u'\sim U}\alpha_u\overline{\alpha_{u'}}\!\!\sum_{v\sim V}\beta_v\,\overline{\beta_{v+h'}}\,W\!\left(v;u,u',h'\right),
		\qquad h'=\frac{h}{(u,u')},
	\end{equation}
	with a smooth weight $W$ supported on $v\asymp V$ and obeying $x^j\partial_x^j W\ll_j 1$ (the exact shape of $W$ comes from unfolding the dyadic cutoffs). It suffices to prove the mean bound \eqref{eq:shortshift-mean} for $\sum_{|h|\le H}|\mathcal C(h;U,V)|$ and then sum over $O((\log N)^3)$ dyadic blocks (absorbed in $(\log N)^B$).

	\emph{Step 2: Balanced prime amplifier kills the diagonal.}
	Fix $P:=V^{\vartheta}$ with some $0<\vartheta<\frac12$ (to be optimised; any fixed choice works). Let $\mathcal P=\{p\ \text{prime}:P\le p\le 2P\}$, and choose signs $\varepsilon_p\in\{\pm1\}$ as in Lemma~\ref{lem:balanced-signs} so that $\sum_{p\in\mathcal P}\varepsilon_p=0$ and $\sum_{p\in\mathcal P}\varepsilon_p\varepsilon_{p+\Delta}\ll |\mathcal P|\mathbf 1_{|\Delta|\le P^{1-o(1)}}$. Consider the amplified second moment
	\[
		\mathcal S\ :=\ \sum_{|h|\le H}\ \Bigg|\ \sum_{p\in\mathcal P}\varepsilon_p\!\!\sum_{v\sim V}\beta_v\,\overline{\beta_{v+h'}}\,W\!\left(v;u,u',h'\right)\,e\!\left(\frac{v\overline{p}}{q}\right)\Bigg|,
	\]
	where the harmless additive twist $e(v\bar p/q)$ is a standard device to linearise shifts after the spectral transform (it is absorbed into the test function and does not change magnitudes; cf. \cite[\S16.2–\S16.4]{IK}). Expanding the square and averaging over $p$ removes the $v=v'$ diagonal thanks to $\sum_{p}\varepsilon_p=0$ and forces a \emph{short shift} $|v-v'|\ll P^{1-o(1)}$ by Lemma~\ref{lem:balanced-signs}.

	\emph{Step 3: Kuznetsov at level $qr$ with level–uniform kernel.}
	After Voronoĭ in $v$ (or directly unfolding against the additive twist) and opening the congruence $u'v'=uv+h$, one arrives at bilinear forms in Kloosterman sums of the prototype
	\[
		\sum_{m\asymp X}\ \sum_{\Delta\ll P}\ S(m,m+\Delta;c)\,W_{u,u',h}(m,\Delta;c),
		\qquad c\equiv 0\pmod{q},
	\]
	with $X\asymp UV/N\asymp 1$ or $X\asymp U$ depending on the chosen unfolding, and with a smooth weight $W_{u,u',h}$ whose $c$–, $m$– and $\Delta$–derivatives are uniformly controlled by powers of $P^{-1}$ (coming from the amplifier) and $U^{-1}$ (coming from smoothing in $u,u'$). Applying the Kuznetsov formula on $\Gamma_0(q)$ (Section~\ref{sec:kuznetsov-uniform}) with the test function from Sublemma~\ref{sub:kernel} places the Bessel argument $z=4\pi \sqrt{mn}/c$ at scale $\ll 1$ provided $c\asymp qU$ and $P\ll V^{1/2-\kappa}$, which we ensure by taking $\vartheta<\tfrac12$. The spectral side is bounded by the spectral large sieve together with the uniform kernel bounds of Sublemma~\ref{sub:kernel}.

	Concretely (see \cite{DeshouillersIwaniec1982} and \cite[Ch.~16]{IK} for this pipeline), one obtains for any $\varepsilon>0$,
	\begin{equation}\label{eq:kuz-bound}
		\sum_{\Delta\ll P}\ \Big|\sum_{m\asymp X} S(m,m+\Delta;c)\,W_{u,u',h}(m,\Delta;c)\Big|
		\ \ll\ P^{1+\varepsilon}\,X^{1-\eta}\,c^{1/2+\varepsilon},
	\end{equation}
	for some $\eta=\eta(\delta)>0$, uniformly in $u,u',h$ and the level $q\le N^{1/2-\varepsilon}$. (This is the classical “short-shift saving” originating in \cite{DeshouillersIwaniec1982}, in the level–uniform form recorded in \cite[\S16.4]{IK}; see also \cite{DukeFriedlanderIwaniec1993,DukeFriedlanderIwaniec1994,FouvryIwaniec1997}.)

	\emph{Step 4: Summing the parameters.}
	Undoing the spectral transform and the amplifier (absorbing the $P^{1+\varepsilon}$ factor and the $|\mathcal P|\asymp P/\log P$ normalisation into $(\log N)^B$), and summing over $u,u'\sim U$ (\,$U,V\in[N^{\delta},N^{1-\delta}]$\,) yields, after Cauchy–Schwarz in $u,u'$,
	\[
		\sum_{|h|\le H}\ |\mathcal C(h;U,V)|\ \ll\ H\,N^{1-\eta}\,(\log N)^{B},
	\]
	with $\eta=\eta(\delta)>0$. Summing the $O((\log N)^3)$ dyadic blocks completes the proof of \eqref{eq:shortshift-mean}.
\end{proof}

\begin{proposition}[Kernel-weighted short shifts]\label{prop:kernel-short}
	Let $K_Q(h)$ be as in Lemma~\ref{lem:kernel-smooth} with a smooth $h_Q$, and $C(h)$ as in
	Lemma~\ref{lem:shortshift-TypeII}. Let $H:=\lfloor N/Q\rfloor$. Then
	\[
		\sum_{|h|\le H}\, K_Q(h)\,C(h)\ \ll\ Q^2\,N^{1-\eta}\,(\log N)^{B}.
	\]
\end{proposition}
...
\begin{proof}
	By \eqref{eq:KQ-short}, $\max_{|h|\le H} |K_Q(h)|\ll Q^2/H+Q^{1+\varepsilon}\ll Q^2/H$ since $H=N/Q\ge Q^{\varepsilon}$.
	Hence
	\[
		\sum_{|h|\le H}\! |K_Q(h)\,C(h)|\ \le\ \Big(\max_{|h|\le H}|K_Q(h)|\Big)\cdot \sum_{|h|\le H}\! |C(h)|
		\ \ll\ \frac{Q^2}{H}\cdot H\,N^{1-\eta}(\log N)^{B}
		\ =\ Q^2\,N^{1-\eta}(\log N)^{B}.
	\]
\end{proof}


\begin{lemma}[Long-shift tail]\label{lem:long-tail}
	With $H=N/Q$ as above,
	\[
		\sum_{|h|>H}\, K_Q(h)\,C(h)\ \ll\ NQ\,(\log N)^{A},
	\]
	for any fixed $A>0$, provided $c_n$ is of Type~I/II and we exclude the pretentious characters via your Halász-distance
	split (Lemmas~\ref{lem:halasz-smooth}, \ref{lem:logfree-density}, \ref{lem:siegel}).
\end{lemma}

\begin{proof}
	Set $H=N/Q$. By Cauchy–Schwarz and the tail bound \eqref{eq:KQ-tail} (take any fixed $A_1\ge 2$),
	\begin{equation}\label{eq:CS-longtail}
		\sum_{|h|>H}\!|K_Q(h) C(h)|
		\ \le\ \Big(\sum_{|h|>H}\! |K_Q(h)|\Big)\cdot \sup_{h}|C(h)|
		\ \ll\ Q^2\Big(\frac{Q}{H}\Big)^{A_1}\ \sup_{h}|C(h)|.
	\end{equation}
	For Type~I/II coefficients we have the crude but uniform $L^2$–bound
	\begin{equation}\label{eq:L2-cn}
		\sum_{n\asymp N} |c_n|^2\ \ll\ N\,(\log N)^{B},
	\end{equation}
	for some absolute $B$ (expand $c_n$ and use that $|\alpha_u|,|\beta_v|\ll\tau^{O(1)}$ with $UV\asymp N$). Hence, for every $h$,
	\begin{equation}\label{eq:Ch-sup}
		|C(h)|\ =\ \Big|\sum_{n\asymp N} c_n\,\overline{c_{n+h}}\Big|\ \le\ \Big(\sum_{n}|c_n|^2\Big)^{1/2}\Big(\sum_{n}|c_{n+h}|^2\Big)^{1/2}\ \ll\ N\,(\log N)^{B}.
	\end{equation}
	Insert \eqref{eq:Ch-sup} into \eqref{eq:CS-longtail} and recall $H=N/Q$ to get
	\[
		\sum_{|h|>H}\!|K_Q(h) C(h)|\ \ll\ N\,(\log N)^{B}\cdot Q^2\Big(\frac{Q}{H}\Big)^{A_1}
		\ =\ N\,(\log N)^{B}\cdot Q^2\Big(\frac{Q^2}{N}\Big)^{A_1}.
	\]
	Since $Q\le N^{1/2-\varepsilon}$, we have $(Q^2/N)^{A_1}\ll N^{-A_1\varepsilon}$, and in particular the right-hand side is $\ll NQ(\log N)^{A}$ after enlarging $A$ (because $Q^3\le NQ$ in the stated range). This proves the lemma.
	\smallskip

	\noindent\emph{Remark.} One may alternatively bound the weighted mean square $\sum_{|h|>H}|K_Q(h)|\,|C(h)|^2$ by opening $C(h)$ and applying the multiplicative large sieve (e.g. \cite[Ch.~7]{MV}, \cite[\S28]{IK}) after the dispersion identity, together with the pretentious split (Lemmas~\ref{lem:halasz-smooth}, \ref{lem:logfree-density}, \ref{lem:siegel}). This yields the same $NQ(\log N)^{A}$ bound.
\end{proof}
