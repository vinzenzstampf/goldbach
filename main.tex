...
\begin{theorem}[BV with parity, second moment]\label{thm:BVP2M}
	Fix $A>0$ and $\varepsilon>0$. Let $N\ge 3$ and $Q\le N^{1/2-\varepsilon}$.
	Let $w:\mathbb R\to\mathbb R_{\ge 0}$ be a smooth weight supported on $[1/2,2]$ with $w^{(j)}\!\ll_j 1$,
	and let $c_n$ be coefficients of the form $c_n = f(n)\,w(n/N)$ with $|f(n)|\ll_\delta \tau(n)^\delta$ for some fixed $\delta>0$.
	Then
	\[
		\sum_{q\le Q}\ \sum_{\chi\!\!\!\pmod q}
		\Bigg|\ \sum_{n\ge 1} c_n\,\lambda(n)\,\chi(n)\ \Bigg|^2
		\ \ \ll_{A,\varepsilon,\delta}\ \ \frac{NQ}{(\log N)^A}\,.
	\]
	The implicit constant depends at most on $A,\varepsilon,\delta$ and on derivative bounds for $w$.
\end{theorem}
...
\begin{lemma}[Halász mean-value bound; e.g. {\cite[Thm.~12.13]{IK}}]\label{lem:halasz}
	Let $g$ be a completely multiplicative function with $|g(n)|\le 1$. Then, for $x\ge 2$,
	\[
		\sum_{n\le x} g(n)
		\ \ll\ x\,\exp\!\big(-\mathcal D(g;x)\big)\ +\ \frac{x}{(\log x)^{100}},
	\]
	where
	$\displaystyle \mathcal D(g;x)\ :=\ \min_{|t|\le x}\ \sum_{p\le x}\frac{1-\Re\big(g(p)p^{-it}\big)}{p}$.
\end{lemma}

\begin{lemma}[Distance for $\lambda\chi$]\label{lem:distance}
	For any Dirichlet character $\chi$ and any $x\ge 3$,
	\[
		\min_{|t|\le x}\ \sum_{p\le x}\frac{1-\Re\!\big(\lambda(p)\chi(p)p^{-it}\big)}{p}
		\ \ge\ \tfrac12\,\log\log x\ +\ O(1).
	\]
\end{lemma}

\begin{proof}[Sketch proof of Lemma~\ref{lem:distance}]
	Since $\lambda(p)=-1$, the summand equals $\frac{1+\Re(\chi(p)p^{-it})}{p}$. Mertens gives $\sum_{p\le x}\frac1p=\log\log x+M+o(1)$. It remains to show
	$\sum_{p\le x}\frac{\Re(\chi(p)p^{-it})}{p}=o(\log\log x)$ uniformly in $\chi,t$.
	For nonprincipal $\chi$, this follows from the prime number theorem in arithmetic progressions with the classical zero-free region and partial summation; for a potential exceptional real $\chi$ one uses Page’s theorem to isolate at most one modulus $q_0$ and obtains the same bound with an absolute implied constant (cf. \cite[Ch.~11-12]{IK}, \cite[Ch.~12]{MV}). For the principal character, $\sum_{p\le x}p^{-1}\cos(t\log p)$ is $o(\log\log x)$ uniformly in $|t|\le x$ by Dirichlet’s test and the oscillation of $\cos(t\log p)$. Details appear for instance in \cite[§12.1-12.2]{IK}.
\end{proof}
...
\begin{sublemma}[Uniform kernel bounds]\label{sub:kernel}
	With notation as above, the Bessel transforms $K_\nu(\mathcal W)$ appearing in Kuznetsov satisfy
	\[
		\sum_{\substack{\text{forms }u\\ \text{on }\Gamma_0(r)}} \frac{|\rho_u(1)|^2}{\cosh(\pi t_u)}\
		\Big|K_{it_u}(\mathcal W)\Big|^2
		\ \ \ll\ \ (Q^2+X)^{1-\delta_0}\,,
	\]
	uniformly in $r\ll X^{o(1)}$, where the sum runs over Maaß forms with Laplace eigenvalue $1/4+t_u^2$, and analogous bounds hold for the holomorphic and Eisenstein parts.
\end{sublemma}

\begin{proof}[Reference]
	This is standard once $\mathcal W$ has derivative control matching the ranges $m,n\asymp X$ and $c\asymp Q$; see e.g.\ Deshouillers-Iwaniec \cite{DI82,DI83} and Blomer-Harcos \cite[Thm.~1]{BH10} for level-uniform spectral large sieve/Kuznetsov with Bessel weight analysis, together with \cite[\S16]{IK}. The short-shift smooth cutoff enters only through bounds on derivatives of $\mathcal W$.
\end{proof}

\begin{sublemma}[van der Corput in short shifts]\label{sub:vdc}
	Let $(\varepsilon_p)$ be as in Lemma~\ref{lem:passg}. Then for any family of phases $\theta_\Delta$ with derivative control in $\Delta$, one has the dispersion estimate
	\[
		\sum_{|\Delta|\le P^{1-\kappa}}\ \Big|\ \sum_{p\in \mathcal P}\varepsilon_p\, e(\theta_{\Delta+p}-\theta_\Delta)\ \Big|^2
		\ \ \ll\ |\mathcal P|^{\,2-\delta_1}\,,
	\]
	for some $\delta_1=\delta_1(\kappa,\vartheta)>0$.
\end{sublemma}

\begin{proof}[Reference]
	A standard van der Corput/weyl-shift argument with the balanced Rademacher property of $(\varepsilon_p)$ and derivative control in $\Delta$; cf.\ the dispersion method as in \cite[\S13]{IK} and the treatment of short-shifted convolutions in \cite{DFI93}. Any prime-by-prime model with $|\mathcal P|\asymp P/\log P$ yields the stated power saving.
\end{proof}
...
\begin{theorem}[Minor-arc $L^2$ bound]\label{thm:minorA1_proved}
	For any $\varepsilon>0$ there exists $\eta=\eta(\varepsilon)>0$ such that
	\[
		\int_{\mathfrak m} |S(\alpha)-B(\alpha)|^2\,d\alpha
