...
\begin{proof}[Proof using Lemma~\ref{lem:S2.4}]
	...
	Let $\mathcal P$ be the set of primes $p\in[P,2P]$ with $P=X^\vartheta$ (to be chosen; PASSG is uniform in $P$).
	...
	Precisely this OD piece is what \textbf{PASSG} estimates \emph{after} the amplifier and Kuznetsov:

	\noindent\textbf{PASSG (assumed).} Uniformly in $P=X^\vartheta$,
...
\begin{itemize}
	\item \textbf{Uniformity \& hypotheses.} The argument only used (i) Type-III structure (smooth $\alpha_n$, divisor bounds), (ii) balanced prime amplifier with $\sum \varepsilon_p=0$, (iii) Kuznetsov with full continuous and oldform ranges, and (iv) PASSG’s OD estimate. No further spectral gap input is needed beyond what S2.4 encapsulates.
...
This completes Part C.5 once PASSG is rigorously in place.
